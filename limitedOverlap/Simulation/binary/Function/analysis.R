...
DIREC_ROOT="C:/Users/Tingting.Zhou/Desktop/paper2/resubmission/binary/"


############################################
source(paste0(DIREC_ROOT, "Function/truth.R"))

#varying the value of gammaV for different degrees of overlap
truthVal=NULL
###correctly specified propensity score model
truthVal=truth(gammaV=4, interceptV=0.4, sampleSize=200000, set.num=1, modelSpec="both")

###misspecified propensity score model
truthVal.mis=truth(gammaV=4, interceptV=0.4, sampleSize=200000, set.num=1, modelSpec="misWeight")
...
for(modelSpec in c("misPred", "misWeight", "both")){
  ...
  DIRECR=paste0(DIREC_ROOT, "homoT/")  ###simulation result
  ...
  #########misspecified propensity score model
  if(modelSpec == "misWeight"){
    
    truth.ATE=truthVal.mis$ATE
    truth.ATM=truthVal.mis$ATM
    truth.ATT=truthVal.mis$ATT
    truth.ATC=truthVal.mis$ATC
    truth.ATO=truthVal.mis$ATO
    truth.truncate=truthVal.mis$truncate
    truth.truncateQ=truthVal.mis$truncateQ
    
  } else {  ###correctly specified propensity score model
    
    truth.ATE=truthVal$ATE
    truth.ATM=truthVal$ATM
    truth.ATT=truthVal$ATT
    truth.ATC=truthVal$ATC
    truth.ATO=truthVal$ATO
    truth.truncate=truthVal$truncate
    truth.truncateQ=truthVal$truncateQ
  ...
  truth=truth.ATE
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATE_all=rbind(processResult(result=ATE, truth = truth),
                processResult(result=ATE.aug, truth),
                processResult(result=ATE_pencomp[, selName], truth = truth))
  ...
  ATM_all=rbind(processResult(result=ATM, truth = truth),
                ...
                processResult(result=ATM_pencomp[, selName], truth = truth),
                processResult(result=ATM_w_pencomp[, selName], truth = truth) )
  ...
  ATO_all=rbind(processResult(result=ATO, truth = truth),
                ...
                processResult(result=ATO_pencomp[, selName], truth = truth) )
  ...
  ATT_all=rbind(processResult(result=ATT, truth = truth),
                ...
                processResult(result=ATT_pencomp[, selName], truth = truth),
                processResult(result=ATT_w_pencomp[, selName], truth = truth) )
  ...
  ATC_all=rbind(processResult(result=ATC, truth = truth),
                ...
                processResult(result=ATC_pencomp[, selName], truth = truth),
                processResult(result=ATC_w_pencomp[, selName], truth = truth) )
  ...
  for(k in 1:length(truncateVal)){
    ...
    selCol_pencomp=paste0(c("estimate", paste0(c("std_", "lowerCI_","upperCI_"), indice)), format(truncateVal[k], digits = 2))
    selCol=paste0(c("estimate", "std", "lowerCI","upperCI"), format(truncateVal[k], digits = 2))
    
    truth=truth.truncate[which(names(truth.truncate)==paste0("truncate", format(truncateVal[k], digits = 2)))]
    ...
    temp=rbind(processResult(result=truncate[, selCol], truth = truth), 
               ...
               processResult(result=truncate_pencomp[, selCol_pencomp], truth = truth) )
  ...
  for(k in 1:length(truncateQVal)){
    ...
    selCol_pencomp=paste0(c("estimate", paste0(c("std_", "lowerCI_","upperCI_"), indice)), format(truncateQVal[k], digits = 2))
    
    truth=truth.truncateQ[which(names(truth.truncateQ)==paste0("truncateQ", format(truncateQVal[k], digits = 2)))]
    ...
    temp=rbind(processResult(result=truncateQ[, selCol], truth = truth), 
               ...
               processResult(result=truncateQ_pencomp[,selCol_pencomp], truth = truth) )
  ...
  write.table(output, paste0(DIRECOUT, modelSpec, "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
...
for(modelSpec in c("misPred2")){
  ...
  DIRECR=paste0(DIREC_ROOT, "homoT/")  ###directory of simulation result
  ...
  truth=truth.ATE
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATE_all=rbind(processResult(result=ATE_pencomp[, selName], truth = truth))
  ...
  truth=truth.ATM
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATM_all=rbind(processResult(result=ATM_pencomp[, selName], truth = truth),
                processResult(result=ATM_w_pencomp[, selName], truth = truth) )
  ...
  truth=truth.ATO
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATO_all=rbind(processResult(result=ATO_pencomp[, selName], truth = truth) )
  ...
  truth=truth.ATT
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATT_all=rbind(processResult(result=ATT_pencomp[, selName], truth = truth),
                processResult(result=ATT_w_pencomp[, selName], truth = truth) )
  ...
  truth=truth.ATC
  selName= c("estimate", paste0(c("std_", "lowerCI_", "upperCI_"), indice))
  ATC_all=rbind(processResult(result=ATC_pencomp[, selName], truth = truth),
                processResult(result=ATC_w_pencomp[, selName], truth = truth) )
  ...
  for(k in 1:length(truncateVal)){
    ...
    temp=rbind(processResult(result=truncate_pencomp[, selCol_pencomp], truth = truth) )
    
    row.names(temp)=paste0(c( "truncate pencomp"), 
                           format(truncateVal[k], digits = 2))
  ...
  for(k in 1:length(truncateQVal)){
    ...
    temp=rbind(processResult(result=truncateQ_pencomp[,selCol_pencomp], truth = truth) )
...
misPred2=read.table(paste0(DIRECOUT, "misPred2", "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
misPred=read.table(paste0(DIRECOUT, "misPred", "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
both=read.table(paste0(DIRECOUT, "both", "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
misWeight=read.table(paste0(DIRECOUT, "misWeight", "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
...
biasT=function(data, var.name="bias"){
  data[,"biasT"] = format(abs(data[, var.name]) * 1000, digits = 2)
  return(data)
}

########multiple bias percentage by 100
biasPer=function(data, var.name="biasPercent"){
  data[,"biasPer"] = format(abs(data[, var.name]) * 100, digits = 2)
  return(data)
}
...
methods=c("ATE", "ATE aug" , "pencomp" ,"ATM"  , "ATM aug" ,               
          "pencomp ATM",  "ATO" ,  "ATO aug" , "pencomp ATO", "ATT",   "ATT aug", 
          "pencomp ATT",  "ATC"  ,                  
          "ATC aug" , "pencomp ATC" , 
          
          "truncate rest0.01", "truncate rest0.05",
          "truncate.aug rest0.01", "truncate.aug rest0.05",
          "truncate pencomp0.01", "truncate pencomp0.05",
          
          "truncateQ rest0", "truncateQ rest0.005",
          "truncateQ.aug rest0", "truncateQ.aug rest0.005",
...
misPred2=misPred2[which(row.names(misPred2) %in% methods), ]
misWeight=misWeight[which(row.names(misWeight) %in% methods), ]

row.names(both)==row.names(misWeight)
...
misPredResult=rbind(misPred["ATE",], misPred["ATE aug",], misPred["pencomp",], misPred2["pencomp",],
                    ...
                    misPred["ATM",], misPred["ATM aug",], misPred["pencomp ATM",], 
                    misPred2["pencomp ATM",], 
                    
                    misPred["ATO",], misPred["ATO aug",], misPred["pencomp ATO",],
                    misPred2["pencomp ATO",],
                    
                    misPred["ATT",], misPred["ATT aug",], misPred["pencomp ATT",], 
                    misPred2["pencomp ATT",],
                    
                    misPred["ATC",], misPred["ATC aug",], misPred["pencomp ATC",], 
                    misPred2["pencomp ATC",], 
                    
                    misPred["truncate rest0.01",],  misPred["truncate.aug rest0.01",],
                    misPred["truncate pencomp0.01",],
                    misPred2["truncate pencomp0.01",],
                    
                    misPred["truncate rest0.05",], misPred["truncate.aug rest0.05",],
                    misPred["truncate pencomp0.05",],
                    misPred2["truncate pencomp0.05",],
                    
                    misPred["truncateQ rest0",], misPred["truncateQ.aug rest0",],
                    misPred["truncateQ pencomp0",],
                    misPred2["truncateQ pencomp0",],
                    
                    misPred["truncateQ rest0.005",], misPred["truncateQ.aug rest0.005",],
...
pdf(paste0(DIREC_ROOT, "Results/plot.pdf"))
...
modelSpec="misPred2"
output500=read.table(paste0(DIRECOUT, modelSpec, "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")
output500$absBias=abs(output500$bias)

###for weighted estimators
modelSpec="misPred"
output500_w=read.table(paste0(DIRECOUT, modelSpec, "_gammaV", gammaV, "_sampleSize", sampleSize, "_indice", indice, ".txt"), sep="\t")

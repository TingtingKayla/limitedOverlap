  ...
  tryCatch ( 
    {
      ...
      y0.model <- glm(modely0, data=data[treatInd==0,], family = "binomial")
      #######################################################################
      ###prediction model for Y under treatment using treated data
      y1.model <- glm(modely1, data=data[treatInd==1,], family = "binomial")
      
      pred0 = predict(y0.model, newdata=data, type = "response")
      pred1 = predict(y1.model, newdata=data, type = "response")
      
      #pred0_prob=predict(y0.model, newdata=data, type = "response")
      #pred1_prob=predict(y1.model, newdata=data, type = "response")
      
      #pred0 = rbinom(nrow(data), 1, prob=pred0_prob)
      #pred1 = rbinom(nrow(data), 1, prob=pred1_prob)

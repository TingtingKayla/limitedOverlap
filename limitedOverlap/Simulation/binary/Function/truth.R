...
truth=function(gammaV=4, interceptV=0.4, sampleSize=200000, set.num=1, modelSpec="misWeight"){
  ...
  betaV=c(0, -0.5, -0.5, -1.5, 0.8, 0.8, 1.0)*2 ###beta0 intercept 0
  treatEff=2.75  ###treatment effect delta
  ...
  y0.lin = outcomeY0Design %*% matrix(betaV, ncol=1)
  y1.lin = outcomeY1Design %*% matrix(c(betaV, treatEff), ncol=1)
  
  y1.prob <- exp(y1.lin)/(1 + exp(y1.lin)) ###probability
  y0.prob <- exp(y0.lin)/(1 + exp(y0.lin)) ##probability 
  
  mydata$y1 = rbinom(n, 1, prob = as.vector(y1.prob)) ### realization
  mydata$y0 = rbinom(n, 1, prob = as.vector(y0.prob))
